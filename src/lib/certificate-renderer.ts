...
export async function renderCertificatePng(input: RenderCertificateInput) {
  ...
  const overlay = `
    <svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" xmlns="http://www.w3.org/2000/svg">
      ...
      <text x="45%" y="52%" text-anchor="start" class="name">${safeParticipantLines[0] ?? "Peserta"}</text>
      ${safeParticipantLines[1] ? `<text x="33%" y="58.8%" text-anchor="start" class="name">${safeParticipantLines[1]}</text>` : ""}
 
      <!-- Nama Pelatihan & Bidang -->
      <text x="45%" y="73%" text-anchor="start" class="value">${escapeSvgText(input.trainingName)}</text>
